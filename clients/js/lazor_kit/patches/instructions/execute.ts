...
export function getExecuteInstructionDataDecoder(): Decoder<ExecuteInstructionData> {
  return {
    decode(bytes) {
      return {
        discriminator: bytes[0] ?? EXECUTE_DISCRIMINATOR,
...
/** Payload after the u8 execute discriminator. */
export function decodeExecutePayload(data: ReadonlyUint8Array): ReadonlyUint8Array {
  if (data.length === 0 || data[0] !== EXECUTE_DISCRIMINATOR) {
    throw new Error("Not an Execute instruction");
  }
  return data.slice(1);
}
