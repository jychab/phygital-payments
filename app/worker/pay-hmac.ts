/**
 * HMAC-tagged pay keys: `ppk_<wallet>_<gen>_<hmac>`.
 * Verification is pure crypto — no database lookup.
 */
...
export type ParsedPayKey = { wallet: string; gen: number };

/** Sign a pay key for `wallet` at generation `gen`. */
export async function signPayKey(
  secret: string,
  wallet: string,
  gen: number,
): Promise<string> {
...
/**
 * Verify an HMAC-tagged pay key. Returns `{ wallet, gen }` or `null`.
 * Pure crypto — timing-safe via double-HMAC comparison.
 */
export async function verifyPayKey(
  secret: string,
  apiKey: string,
): Promise<ParsedPayKey | null> {
