...
/** True when the batch only creates a wallet (+ budget / rent helpers). */
export function isCreateWalletOnlyBatch(
  ...
  for (const ix of instructions) {
    ...
    if (LAZORKIT_PROGRAMS.has(program)) {
      if (matchesU8Discriminator(ix.data, CREATE_WALLET_DISCRIMINATOR)) {
        sawCreateWallet = true;
        continue;
      }
      return false;
    }
    if (program === String(SYSTEM_PROGRAM_ADDRESS)) continue;
