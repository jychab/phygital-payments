...
async function fetchAllDasAssets(ownerAddress: string): Promise<{
  ...
  if (!first) return { items: [], nativeBalance: undefined, pageCount: 0 };
  ...
  if (pageCount <= 1) {
    return { items: firstBatch, nativeBalance, pageCount: 1 };
  }
  ...
  return { items, nativeBalance, pageCount };
}

export async function fetchWalletPortfolio(
  vaultPda: string,
): Promise<WalletPortfolio & { dasPageCount?: number }> {
  const { items, nativeBalance, pageCount } = await fetchAllDasAssets(vaultPda);
